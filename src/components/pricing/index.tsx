...
// Constants
import {BILLING_CYCLES, PRICES_URL, DEFAULT_TAG} from '../../constants';
...
class Pricing extends React.Component<Props, State> {
	...
	get basePrice(): number {
		...
		const {selectedProductType, selectedProduct} = this.state;
		const selectedProductKey =
			selectedProductType === 'shared'
				? `shared_${selectedProduct}`
				: selectedProduct;
		const selectedProductBasePrice =
			(prices[selectedProductKey] && prices[selectedProductKey].price) ||
			0;
	...
	get quantityDiscount(): number {
		const {
			discounts: {quantity: quantityDiscounts}
		} = this.props;
	...
	get billingCycleDiscount(): number {
		const {
			discounts: {interval: billingCycleDiscounts}
		} = this.props;
		const {selectedBillingCycle} = this.state;

		return selectedBillingCycle !== 1 && billingCycleDiscounts
			? parseFloat(billingCycleDiscounts[selectedBillingCycle])
			: 0;
	}
	...
	get priceWithoutDiscount(): number {
		...
		return (
			...
			selectedBillingCycle *
			selectedQuantity *
			parseFloat(selectedTag.multiplier)
		);
	}
	...
	get totalDiscount() {
		...
		return (
			((this.priceWithoutDiscount - parseFloat(this.priceWithDiscount)) *
				100) /
	...
	get biggestQuantityDiscount() {
		...
		const quantityDiscountValues = discounts.quantity
			? Object.keys(discounts.quantity)
			: [];

		return quantityDiscountValues.length
			? parseInt(
					quantityDiscountValues[quantityDiscountValues.length - 1]
			  )
			: 0;
	}
	...
	get tagsByService() {
		...
		const selectedProductTags =
			prices[selectedProductKey] && prices[selectedProductKey].tags;
		...
		return (
			selectedProductTags.map(({name, price_multiplier}) => ({
				value: name,
				label: name,
						...
						<Col xs={{size: 12}}>
							<div className="pricing-card">
								<Preamble title="Lowest prices in the industry">
									Our prices have always been competitive and
									we plan on keeping them that way. On top of
									that, we always have discounts based on
									quantity or billing period so make sure you
									play with our pricing calculator below to
								...
								<div className="slider">
									<Slider
										...
										min={1}
										max={
											this.biggestQuantityDiscount || 100
										}
										onChange={this.handleChange.bind(
											this,
											'selectedQuantity'
								...
								<footer className="pricing-card__footer d-flex">
									<div>
										<div className="pricing-card__footer-value text-left">
											{selectedQuantity}
										</div>
										<div className="pricing-card__footer-label text-left">
											Quantity
										</div>
									</div>
									<div>
									...
									<div className="d-flex pricing-card__footer-total">
										<div>
											<div className="pricing-card__footer-value pricing-total text-left">
...
const mapDispatchToProps = dispatch => ({
	doSetPrices: (pricing: IPricingResponse) =>
